<div id="erd" class="erd-tab-content active" style="padding: 1rem; background: #fafafa; border-radius: 4px; overflow: auto;"></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        const viz = new Viz();
        viz.renderString(erdDot).then(svg => {
            document.getElementById('erd').innerHTML = svg;
        }).catch(e => {
            console.error('Viz.js error:', e);
            document.getElementById('erd').innerHTML = '<p>Error rendering diagram</p>';
        });
    } catch(e) {
        console.error('Viz.js error:', e);
        document.getElementById('erd').innerHTML = '<p>Error rendering diagram</p>';
