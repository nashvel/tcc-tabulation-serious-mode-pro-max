...
flowchart TD
    ...
    SetActiveRound --> BroadcastWS[Broadcast WebSocket<br/>voting-state channel<br/>with active_round via FK]
...
flowchart TD
    ...
    SaveResponse -->|Yes| ShowSuccess[Show Success Toast<br/>Category from candidate.gender]
...
graph TB
    ...
    subgraph Database["Database Tables"]
        ...
        JudgeSessions["judge_sessions<br/>id, judge_id<br/>session_token, status"]
        Candidates["candidates<br/>id, name, gender"]
        Partnerships["candidate_partnerships<br/>id, candidate_id<br/>partner_name"]
        Rounds["rounds<br/>id, event_id<br/>name, spot"]
        Criteria["criteria<br/>id, name, points"]
        Points["points<br/>candidate_id<br/>judge_id FK<br/>points"]
        VotingState["voting_states<br/>is_active<br/>active_round_id FK"]
    ...
    style WS fill:#9C27B0,color:#fff
        ...
        <div id="database-schema" class="diagram-section">
            ...
            <div class="info-box">
                <h3>Normalization Changes (Nov 25, 2025):</h3>
                <ul>
                    <li><strong>New Tables:</strong>
                        <ul>
                            <li><code>judges</code> - Judge management with chair assignments</li>
                            <li><code>judge_sessions</code> - Real-time judge session tracking</li>
                            <li><code>candidate_partnerships</code> - Separated partner data (1NF compliance)</li>
                        </ul>
                    </li>
                    <li><strong>Removed Redundancy:</strong>
                        <ul>
                            <li>Removed <code>active_round_name</code> from <code>voting_states</code> (use FK to rounds)</li>
                            <li>Removed <code>category</code> from <code>points</code> (use candidate.gender)</li>
                            <li>Removed partner columns from <code>candidates</code> (moved to partnerships table)</li>
                        </ul>
                    </li>
                    <li><strong>Added Foreign Keys:</strong>
                        <ul>
                            <li><code>rounds.event_id</code> - Link rounds to events</li>
                            <li><code>points.judge_id</code> - FK constraint for data integrity</li>
                        </ul>
                    </li>
                </ul>
            </div>
...
graph TB
    subgraph Core["Core Tables"]
        Events["events<br/>id, title, year<br/>status, timestamps"]
        EventDays["event_days<br/>id, event_id<br/>day_number, title"]
    end
    
    subgraph Judging["Judging System"]
        Judges["judges<br/>id, event_id<br/>name, chair_number<br/>laptop_ip, status"]
        JudgeSessions["judge_sessions<br/>id, judge_id, event_id<br/>session_token, status<br/>logged_in_at"]
        Points["points<br/>id, candidate_id<br/>judge_id FK<br/>round_id, criteria_id<br/>points"]
    end
    ...
    subgraph Candidates["Candidate Management"]
        ...
        Partnerships["candidate_partnerships<br/>id, candidate_id<br/>partner_number<br/>partner_name<br/>partner_gender"]
    end
    
    subgraph Rounds["Round & Criteria"]
        RoundsTable["rounds<br/>id, event_id FK<br/>spot, name"]
        Criteria["criteria<br/>id, round_id<br/>name, points"]
    end
    
    subgraph Voting["Voting State"]
        VotingState["voting_states<br/>id, event_id<br/>active_round_id FK<br/>active_session_id<br/>is_locked"]
        VotingSessions["voting_sessions<br/>id, event_id<br/>round_id, status<br/>started_at"]
    end
    ...
    style Sequences fill:#FFF9C4
            ...
            <div class="info-box">
                ...
                <ul>
                    <li><strong>events</strong> → All other tables (one-to-many)</li>
                    <li><strong>judges</strong> → judge_sessions (one-to-many)</li>
                    <li><strong>judges</strong> → points (one-to-many)</li>
                    <li><strong>candidates</strong> → candidate_partnerships (one-to-one)</li>
                    <li><strong>candidates</strong> → points (one-to-many)</li>
                    <li><strong>rounds</strong> → criteria (one-to-many)</li>
                    <li><strong>rounds</strong> → points (one-to-many)</li>
                    <li><strong>voting_states.active_round_id</strong> → rounds (FK)</li>
                    <li><strong>points.judge_id</strong> → judges (FK)</li>
                </ul>
            </div>
        </div>
